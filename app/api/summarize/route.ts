  ...
  try {
    ...
    try {
      ...
      chunks = documentPoints
        .sort(
          (a: any, b: any) => a.payload?.chunk_index - b.payload?.chunk_index
        )
      ...
      return NextResponse.json(
        {
          error:
            "Failed to retrieve document from Qdrant: " + qdrantError.message,
        },
        { status: 500 }
      );
    }
          ...
          messages: [
            {
              role: "system",
              content:
                "You are a helpful assistant that creates concise, informative summaries of documents. Provide key points, main ideas, and important details.",
            },
            {
              role: "user",
